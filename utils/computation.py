...
class Computation():
    ...
    def empty_zones(points,tri,area_threshold=4):
        # On récupère la triangulation de Delaunay, et on calcule les aires des triangles : {idx: [point1, point2, point3]}
    ...
    def get_edge_indices(triangle):
        # Retourne les paires d’indices d’arêtes d’un triangle (3 côtés)
        return [(triangle[0], triangle[1]),
                (triangle[1], triangle[2]),
                (triangle[2], triangle[0])]
    def expansion(triangles,tri, points, ratio_threshold=1.5):
        '''On a détecté de premiers triangles, on va également étendre aux triangles voisins, si ceux si répondent au critère : 
        la somme des deux nouveaux côtés est inférieure ratio_threshold fois le coté en commun'''
        ...
        while to_visit:
            ...
            for i, neighbor_idx in enumerate(tri.neighbors[current_idx]):
                ...
                if len(shared) != 2:
                    continue  # doivent partager exactement une arête

                a, b = list(shared)
                edge = edge_length(points[a], points[b])

                # Points restants (deux nouveaux côtés du triangle voisin)
                other_points = list(set(neighbor_triangle) - shared)
                if len(other_points) != 1:
                    continue
                c = other_points[0]

                d1 = edge_length(points[a], points[c])
                d2 = edge_length(points[b], points[c])
